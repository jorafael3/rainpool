...
class Walker_Nav_Menu_Edit_Custom extends Walker_Nav_Menu  {
	...
	function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
	    ...
	    if ( ! empty( $item->_invalid ) ) {
	        ...
	        $title = sprintf( esc_html__( '%s (Invalid)' , 'textron' ), $item->title );
	    } elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
	        $classes[] = 'pending';
	        /* translators: %s: title of menu item in draft status */
	        $title = sprintf( esc_html__('%s (Pending)' , 'textron'), $item->title );
	    }
	            ...
	            <dt class="menu-item-handle">
	                ...
	                <span class="item-controls">
	                    ...
	                    <span class="item-order hide-if-js">
	                        ...
	                        ?>" class="item-move-up"><abbr title="<?php echo esc_attr__('Move up', 'textron'); ?>">&#8593;</abbr></a>
	                        |
	                        <a href="<?php
	                            echo wp_nonce_url(
	                                add_query_arg(
	                        ...
	                        ?>" class="item-move-down"><abbr title="<?php echo esc_attr__('Move down', 'textron'); ?>">&#8595;</abbr></a>
	                    </span>
	        ...
	        <div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
	            <?php if( 'custom' == $item->type ) : ?>
	                <p class="field-url description description-wide">
	                    <label for="edit-menu-item-url-<?php echo esc_attr($item_id); ?>">
	                        <?php echo esc_html__( 'URL', 'textron' ); ?><br />
	            ...
	            <p class="description description-thin">
	                <label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
	                    <?php echo esc_html__( 'Navigation Label', 'textron' ); ?><br />
	                    <input type="text" id="edit-menu-item-title-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
	                </label>
	            </p>
	            <p class="description description-thin">
	                <label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
	                    <?php echo esc_html__( 'Title Attribute', 'textron' ); ?><br />
	            ...
	            <p class="field-link-target description">
	                <label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
	                    ...
	                    <?php echo esc_html__( 'Open link in a new window/tab', 'textron' ); ?>
	                </label>
	            </p>
	            <p class="field-css-classes description description-thin">
	                <label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
	                    <?php echo esc_html__( 'CSS Classes (optional)', 'textron' ); ?><br />
	                    <input type="text" id="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
	                </label>
	            </p>
	            <p class="field-xfn description description-thin">
	                <label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
	                    <?php echo esc_html__( 'Link Relationship (XFN)', 'textron' ); ?><br />
	                    <input type="text" id="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
	                </label>
	            </p>
	            <p class="field-description description description-wide">
	                <label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
	                    <?php echo esc_html__( 'Description' , 'textron'); ?><br />
	                    <textarea id="edit-menu-item-description-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr($item_id); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
	                    <span class="description"><?php echo esc_html__('The description will be displayed in the menu if the current theme supports it.',  'textron'); ?></span>
	                </label>
	            </p>
	            ...
	            <div class="et-clearfix">
		            <div class="field-custom megamenu-options description description-wide">
	                	<div class="megamenu-opt mms">
		                	...
		                	<label for="edit-menu-item-megamenu-<?php echo esc_attr($item_id); ?>"><?php echo esc_html__( 'Megamenu','textron' ); ?></label><br/>
		                	<select class="widefull" id="edit-menu-item-megamenu-<?php echo esc_attr($item_id); ?>" name="menu-item-megamenu[<?php echo esc_attr($item_id); ?>]">
				                <option value="false" <?php if($selected2 == "false"){echo 'selected';}?>><?php echo esc_html__('false','textron'); ?></option>
				                <option value="true" <?php if($selected2 == "true"){echo 'selected';}?>><?php echo esc_html__('true','textron'); ?></option>
				            </select>
	                	</div>
	                	<div class="megamenu-opt hidden mmc">
		                	<?php $selected = get_post_meta( esc_attr($item_id), '_menu_item_megamenu_content', true ); ?>
		                	<label for="edit-menu-item-megamenu_content-<?php echo esc_attr($item_id); ?>"><?php echo esc_html__( 'Megamenu content','textron' ); ?></label><br/>
		                	...
		                	<?php if ($megamenu->have_posts()): ?>
		                		...
		                		<option value="select" <?php if($selected == "select"){echo 'selected';}?>><?php echo esc_html__('-- Select -- ','textron'); ?></option>
		                	...
		                	<?php else: ?>
		                		<p><?php echo esc_html__('First you must create a megamenu from ','textron'); ?> <a href="<?php echo esc_url(home_url('/')).'wp-admin/post-new.php?post_type=megamenu'; ?>"><?php echo esc_html__('here','textron'); ?></a></p>
		                	<?php endif ?>

			            </div>
			            <p class="field-custom">
			                <label for="edit-menu-item-ltext-<?php echo esc_attr($item_id); ?>">
			                    <?php echo esc_html__( 'Submenu item label text','textron' ); ?><br />
			            ...
			            <p class="field-custom">
			                <label for="edit-menu-item-ltextcolor-<?php echo esc_attr($item_id); ?>">
			                    <?php echo esc_html__( 'Submenu item label text color','textron' ); ?>
			                </label>
			            ...
			            <p class="field-custom">
			                <label for="edit-menu-item-lcolor-<?php echo esc_attr($item_id); ?>">
			                    <?php echo esc_html__( 'Submenu item label color','textron' ); ?>
			                </label>
			            ...
			            <p class="field-custom description description-wide">
			                <label for="edit-menu-item-icon-<?php echo esc_attr($item_id); ?>">
			                    <?php echo esc_html__( 'SVG icon url','textron' ); ?><br />
		            ...
		            <div class="menu-item-actions description-wide submitbox">
		                <?php if( 'custom' != $item->type && $original_title !== false ) : ?>
		                    <p class="link-to-original">
		                        <?php printf( esc_html__('Original: %s', 'textron'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
		                    </p>
		                <?php endif; ?>
		                ...
		                ); ?>"><?php echo esc_html__('Remove', 'textron'); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( array('edit-menu-item' => esc_attr($item_id), 'cancel' => time()), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ) );
		                    ?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php echo esc_html__('Cancel', 'textron'); ?></a>
