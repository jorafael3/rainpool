<?php
	textron_enovathemes_global_variables();
 	$blog_single_social         = (isset($GLOBALS['textron_enovathemes']['blog-single-social']) && $GLOBALS['textron_enovathemes']['blog-single-social'] == 1) ? "true" : "false";
 	$blog_authorbox             = (isset($GLOBALS['textron_enovathemes']['blog-authorbox']) && $GLOBALS['textron_enovathemes']['blog-authorbox'] == 1) ? "true" : "false";
			...
			<article <?php post_class() ?> id="post-<?php the_ID(); ?>">
				...
				<?php
                    ...
                    $title_output .='<div class="post-title-section">';
						$title_output .='<div class="post-meta et-clearfix">';
							$title_output .='<div class="post-date">'.get_the_date().'</div>';
							if ('' != get_the_category_list()){
								$title_output .='<div class="post-category">'.get_the_category_list(esc_html__( ', ', 'textron' )).'</div>';
							}
						$title_output .='</div>';
	                        ...
	                        if (has_post_thumbnail()){
	                            ...
	                            $media_output .='<div class="post-image overlay-hover post-media">';
	                                $media_output .='<div class="image-container image-container-single">';
	                                	$media_output .=textron_enovathemes_build_post_media('full','full',false);
	                                $media_output .='</div>';
	                            $media_output .='</div>';
	                        }
                        }
	                            ...
	                            $media_output .='<div class="post-gallery post-media overlay-hover ">';
	                                $media_output .='<ul class="slides tns-slider tns-gallery tns-subpixel tns-calc tns-horizontal">';
	                                    foreach ($gallery as $image => $url){
	                                        $media_output .='<li >';
	                                            $media_output .='<div class="image-container image-container-single">';
	                                                $media_output .=textron_enovathemes_build_post_media('full','full',$image);
	                                            $media_output .='</div>';
	                                        $media_output .='</li>';
	                                    }
	                                $media_output .='</ul>';
                    ...
                    $body_output .='<div class="post-body et-clearfix">';
	                    ...
	                    $body_output .='<div class="post-body-inner">';
							...
							$body_output .='<div class="post-bottom et-clearfix">';
								...
								if (has_tag()) {
									$body_output .='<div class="post-tags-single">'.esc_html__("Tags:", 'textron').' '.get_the_tag_list( '', ' ', '' ).'</div>';
								}
				...
				<div class="post-inner et-clearfix">
					...
					<?php if ($blog_single_sidebar == "none"): ?>
						<?php echo textron_enovathemes_output_html($title_output); ?>
